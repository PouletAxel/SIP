...
public class PeakAnalysisScore {
	...
	/** HashMap of object java.plop.loops*/
	 ...
	 * @param data HashMap of java.plop.loops
	 */
	...
	/**
	 * Method to compute the score of each loop. on a 11*11 square, the average of the corner (3*3) are computed. then the ration between the java.plop.loops value and this avg is computed.
	 * For the regional value, the avg of the n_8 value of the java.plop.loops are done, then a ratio is computed with the avg value of the corner,
	...
	private float process3By3SquareAvg(int x, int y){
		...
		for(int i = x-1; i <= x+1; ++i){
			for(int j = y-1; j <= y+1; ++j){
				if(i < this._ipRaw.getWidth() && i>0 && j < this._ipRaw.getHeight() && j > 0){
					if(	Double.isNaN(this._ipRaw.getf(i, j))){
						sum += 0;
					}else{
	...
	private float computeAvgCorner(int x, int y){
		...
		corner += process3By3SquareAvg(x+10,y-10);
		corner += process3By3SquareAvg(x+10,y+10);
		//System.out.println(corner+" dans avgCorner");
